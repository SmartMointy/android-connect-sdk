...
public class AirPlayAuth {
    ...
    public static String generateNewAuthToken() {
        ...
        final byte[] key = new KeyPairGenerator().generateKeyPair().getPrivate().getEncoded();
        return clientId + "@" + net.i2p.crypto.eddsa.Utils.bytesToHex(key);
    }
     ...
     * @throws IOException
     */
    public void startPairing(Socket socket) throws IOException {
        AuthUtils.performRequest(socket, "POST", "/pair-pin-start", null, null);
    }
     ...
     */
    public void doPairing(Socket socket, String pin) throws Exception {
        ...
        System.out.println("Pairing Sent");
    }
     ...
     */
    public void authenticate(Socket socket) throws Exception {
    ...
    private PairSetupPin2Response doPairSetupPin2(Socket socket, final byte[] publicClientValueA, final byte[] clientEvidenceMessageM1) throws Exception {
        ...
        byte[] pairSetupPin2ResponseBytes = AuthUtils.performRequest(socket, "POST", "/pair-setup-pin", "application/x-apple-binary-plist", pairSetupPinRequestData);
    ...
    private PairSetupPin3Response doPairSetupPin3(Socket socket, final byte[] sessionKeyHashK) throws Exception {
        ...
        byte[] pairSetupPin3ResponseBytes = AuthUtils.performRequest(socket, "POST", "/pair-setup-pin", "application/x-apple-binary-plist", pairSetupPinRequestData);
    ...
    private byte[] doPairVerify1(Socket socket, byte[] randomPublicKey) throws Exception {
        return AuthUtils.performRequest(socket, "POST", "/pair-verify", "application/octet-stream", AuthUtils.concatByteArrays(new byte[]{1, 0, 0, 0}, randomPublicKey, authKey.getAbyte()));
    }
    ...
    private void doPairVerify2(Socket socket, byte[] pairVerify1Response, byte[] randomPrivateKey, byte[] randomPublicKey) throws NoSuchAlgorithmException, NoSuchPaddingException, InvalidKeyException, IOException, InvalidAlgorithmParameterException, SignatureException {
        ...
        AuthUtils.performRequest(socket, "POST", "/pair-verify", "application/octet-stream", AuthUtils.concatByteArrays(new byte[]{0, 0, 0, 0}, signature));
    }
}
